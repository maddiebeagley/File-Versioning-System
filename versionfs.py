...
class VersionFS(LoggingMixIn, Operations):
    ...
    def newVersion(self, path, fh):
        ...
        for version in versions:
            ...
            # only examine the filename with base, not full directory path
            filename = os.path.basename(version)

            # ignore temp files generated by nano
            if not (filename.endswith('.swp')):

                # extract version number from filename and increment
        ...
        # store the most recent version as version 1
